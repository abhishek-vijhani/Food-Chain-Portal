...
require 'menubar.php';
?>
<br /><br /><br /><br /><br /><center><fieldset style="border-bottom: hidden;border-right: hidden;border-left: hidden;width:50%;border-color:black;border-width:3px;"><legend><h2>Fast Food Menu</h2></legend></fieldset></center>
...
$qry="select item_code,item_name,item_price from items where item_code like 'F%'";
...
while($row=mysql_fetch_row($res))
 ...
 echo "<form method='post'>";
 echo "<td><input type='number' name='quantity' min='1' max='50' value='1'/></td>";
 echo "<td><button type='submit' name='temp_add' value='$row[0]'>Add to Cart</button></td>";
 echo "</form>";
 }}
 if(isset($_SESSION['Username'])){
...
{
    if(isset($_POST['temp_add']))
    {
        $code=$_POST['temp_add'];
        $quantity=$_POST['quantity'];
        $a=array();
        array_push($a,$code,$quantity);
        $_SESSION['temp_user']=$a;
        echo "<script>alert('Go to Cart to continue.')</script>";  
    }
}
?>
